...
bot.onText(/\/start/, async (msg) => {
    ...
    try {
        ...
        if (isSubscribed) {
            const photoUrl = 'https://i.ibb.co/GWLD0nX/214876ed20b5.jpg';
            bot.sendPhoto(chatId, photoUrl, {
                caption: `🎉 *Welcome back!* 😊\n\n*Send a TeraBox link to watch or download your video.* 🍿`,
                parse_mode: 'Markdown',
                reply_markup: {
                    inline_keyboard: [
                        [{ text: "Any Help?", url: "https://t.me/l_abani" }]
                    ]
                }
            });
            return;
...
bot.on('message', async (msg) => {
    ...
    try {
        ...
        bot.sendMessage(chatId, `🔄 *Processing your link...*`).then(sentMessage => {
            ...
            axios.get(`https://tera.ronok.workers.dev/?link=${text}&apikey=45f4ed14a9e10627e5697301507c1363b03ef82a`)
                .then(response => {
